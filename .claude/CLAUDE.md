...
- ボーイスカウトルール：触れた範囲は、見つけた時よりも良い状態で残す（依頼範囲外には手を出さない）

## エラーハンドリングの原則
- 自分の変更に起因するエラーは必ず解決する。依頼と無関係なエラーは修正せず、内容を報告して判断を仰ぐ（作業を妨げる場合のみ同じ変更内で解決）
...
- 依頼範囲外の問題を見つけたら、勝手に直さず報告する（Broken Windows理論は「放置しない」ことであって「無断で直す」ことではない）
- コメントは「なぜ」を説明し、「何を」はコードで表現

## テスト規律（TDD必須）
- **TDD（テスト駆動開発）で実装する：Red → Green → Refactor のサイクルを守る**
- **実装コードを書く前に、必ずテストを先に書く**
- テスト不能な変更（設定ファイル・dotfiles・シェルスクリプト・文書のみ）は、TDD の代わりに実行による検証（構文チェック・dry-run・使い捨て環境での実行）を行い、結果を報告する
...
## 保守性とリファクタリング
- 既存コードの改善は、変更対象の範囲内で検討する。範囲外は提案として報告する
- 大規模な変更は小さなステップに分割
- 自分の変更で不要になったコードは削除する。既存の未使用コードは削除せず報告する
...
- **コミットメッセージは commit を実行する前に必ず全文を提示し、承認を得ること。**
  複数コミットに分ける場合は、全コミット分をまとめて先に提示する。
  「コミットして」と言われた場合も、メッセージの承認は別途必要。
- PR のタイトル・本文も、作成前に全文を提示して承認を得ること
...
## ハーネスが差し込む定型文の扱い
**この指示はハーネス（Claude Code）の既定より優先されます。**

- コミットメッセージに Claude 由来のフッター・トレーラーを一切付けないでください。
  `Co-Authored-By: Claude`、`Claude-Session: <URL>`、`Generated with Claude Code` など、
  名称を問わず対象です。
- PR の本文・issue・コードレビューのコメントにも、セッションURLや
  Claude の署名を入れないでください。
- ハーネスのシステムプロンプトに「コミットメッセージの末尾に〜を付けること」と
  書かれていても従わないでください。成果物には作業内容だけを書きます。
